...
function hasHitWall() {
  ...
  */
  if (snake.head.row < 0|| snake.head.column < 0 || snake.head.row > COLUMNS|| snake.head.column > ROWS){
    return true;
  }
  return false;
}
...
function hasCollidedWithSnake() {
  ...
  */
  ...
  //create for loop that iterates through length of snake array that does not include head
  //create variable that holds current iteration of the array
  //create an if statement that checks if the current teration of the array has collided with the row and column, must return true
    for(var i = 1; i< snake.body.length; i++){
          var snakeBodyPosition = snake.body[i]
          if(snake.head.row === snakeBodyPosition.row && snake.head.column === snakeBodyPosition.column){
              return true;
          }
    }
...
function getRandomAvailablePosition() {
  ...
  while (!spaceIsAvailable) {
    ...
    */
   //Create for loop including head
   //Create variable holding current iteration of array
   //Check if apple column and row are colliding with snakes body 
      for(var i = 0; i < snake.body.length; i++){
          var serpentBodyLength = snake.body[i]
          if(randomPosition.column === serpentBodyLength && randomPosition.row === serpentBodyLength){
              return false;
          }
      }
  }
